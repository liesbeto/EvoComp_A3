...
def evolution_brain(label, robot_core_string, terrain="flat", generations=200):
    ...
    # saving: ensure target directories exist and avoid embedding absolute paths in filenames
    logs_dir = Path("__logs__")
    gecks_dir = Path("__gecks__")
    logs_dir.mkdir(parents=True, exist_ok=True)
    gecks_dir.mkdir(parents=True, exist_ok=True)

    # ensure label is a filename only (no directories). Use only the basename.
    safe_label = Path(label).name

    df = pandas_logger.to_dataframe()
    df.to_csv(logs_dir / f"{safe_label}.csv")

    best_solution = searcher.status['best']
    best_policy = problem.make_net(best_solution)
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    torch.save(best_policy.state_dict(), gecks_dir / f"{safe_label}_best_{timestamp}.pth")
    return best_policy, str(gecks_dir / f"{safe_label}_best_{timestamp}.pth")
...
def crossover_and_mutation(robots, scaling_factor=-0.5):
    ...
    for genotype in mutated_genotype_offspring:
        ...
        # store the actual path to the saved robot JSON so later opens find it
...
def main(body_gens, MAX_BRAIN_GENS=200, MIN_BRAIN_GENS=10):
    ...
    # track max fitness per body-generation
    max_fitness_per_generation: list[float] = []

    # prepare incremental CSV writer so results are available even on interrupt
    save_path = DATA / f"{SCRIPT_NAME}_max_fitness.csv"
    import csv
    ...
    try:
        for gen in range(body_gens):
            ...
            print(f"Generation {gen}, Brain gens: {brain_gens}")
            print(robots[0])

            # compute max fitness in this generation (if available)
            fitness_vals = [robots[k][5] for k in robots if len(robots[k]) > 5]
            if fitness_vals:
                max_fitness = float(max(fitness_vals))
            else:
                max_fitness = float('nan')
            max_fitness_per_generation.append(max_fitness)

            # write incremental result immediately
            csv_writer.writerow([gen, max_fitness])
            csvfile.flush()
    ...
    # After evolution, save max-fitness time-series and plot
    # Try to plot final or partial results
    try:
        plt.figure(figsize=(8, 4))
        gens = list(range(len(max_fitness_per_generation)))
        plt.plot(gens, max_fitness_per_generation, marker='o')
        plt.xlabel('Body generation')
        plt.ylabel('Max fitness')
        plt.title('Max fitness per body-generation')
        plt.grid(True)
        plot_path = DATA / f"{SCRIPT_NAME}_max_fitness.png"
        plt.tight_layout()
        plt.savefig(plot_path)
        try:
            plt.show()
        except Exception:
            print(f"Saved CSV to {save_path} and saved plot to {plot_path} but could not display interactively.")
    except Exception as e:
        print(f"Failed to create plot for max fitness: {e}")


if __name__ == "__main__":
    main(body_gens=6, MAX_BRAIN_GENS=5, MIN_BRAIN_GENS=1)
