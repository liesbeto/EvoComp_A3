...
def main(pop: int) -> None:
    ...
    while len(cores) < pop:
        ...
        test_movement(robot=core, controller=ctrl)
        fitness = movement_fitness(tracker.history["xpos"][0])
        print(fitness)
        if fitness > 0:
            cores.append(core)
            ...
            save_graph_as_json(
                robot_graph,
                DATA_ROBOTS / f"robot_graph_{i}_{timestamp}.json"
            )
